...
export function MainAction({ vehicles, size = 'md', full = true, suffix }: { vehicles: Vehicle[]; size?: 'sm' | 'md'; full?: boolean; suffix?: string }) {
  ...
  const base = clsx(
    'inline-flex items-center justify-center gap-1.5 rounded-md font-bold tracking-wide uppercase transition-colors outline-none focus-visible:ring-2 focus-visible:ring-sky-500 touch-manipulation',
    size === 'md' ? 'h-9 px-3 text-[12px]' : 'h-7 px-2 text-[10.5px]',
    full && 'w-full',
  )
  if (list.every((s) => s === 'EN_MISSION'))
    return (
      <button type="button" onClick={() => retour(vehicles.map((v) => v.id))} className={clsx(base, 'bg-ok text-white hover:bg-green-800')}>
        <CornerDownLeft className="size-4" strokeWidth={2.5} /> {suffix ? `Retour ${suffix}` : 'Retour disponible'}
      </button>
    )
  if (list.every((s) => s === 'DISPONIBLE'))
    return (
      <button type="button" onClick={() => sortir(vehicles)} className={clsx(base, 'border border-mission/30 bg-white text-mission hover:border-mission hover:bg-mission hover:text-white')}>
        <Siren className="size-4" strokeWidth={2.5} /> {vehicles.length > 1 ? 'Sortir le départ' : suffix ? `Sortir ${suffix}` : 'Sortir'}
      </button>
    )
  if (vehicles.length === 1)
    return (
      <button type="button" onClick={() => setVehicleState(vehicles[0].id, 'DISPONIBLE')} className={clsx(base, 'border border-line bg-white text-slate-700 hover:bg-slate-50')}>
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className="grid size-7 place-items-center rounded text-slate-400 hover:bg-slate-100 hover:text-slate-700"
      >
        <EllipsisVertical className="size-4" />
      </button>
      {open && (
        <div className="absolute top-8 right-0 z-30 w-48 overflow-hidden rounded-lg border border-line bg-white py-1 shadow-lg">
          <p className="px-3 pt-1.5 pb-1 text-[10.5px] font-semibold tracking-wider text-slate-400 uppercase">Changer l’état</p>
          {(['DISPONIBLE', 'EN_MISSION', 'RESERVE', 'INDISPONIBLE'] as VehicleState[]).map((st) => (
            <button
              ...
              className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 disabled:font-semibold disabled:text-slate-900"
            >
              <span className={clsx('size-2.5 rounded-full', STATE_META[st].dot)} />
              {STATE_LABEL[st]}
              {st === current && <span className="ml-auto text-[11px] text-slate-400">actuel</span>}
            </button>
          ))}
        </div>
      )}
...
function CrewCount({ vehicle }: { vehicle: Vehicle }) {
  ...
  return (
    <span className={clsx('text-[11px] font-semibold tabular-nums', filled < total ? 'text-reserve' : 'text-slate-400')} title="Postes armés">
      {filled}/{total}
    </span>
  )
}
...
export function VehicleCard({ vehicle }: { vehicle: Vehicle }) {
  ...
  return (
    <article
      id={`veh-${vehicle.id}`}
      className={clsx(
        'flex flex-col overflow-hidden rounded-lg border bg-white shadow-[0_1px_2px_rgb(15_29_51/0.06)] transition-shadow',
    ...
    >
      ...
      <header className={clsx('px-3 pt-2 pb-2', out && 'bg-mission-soft/50')}>
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-display text-[17px] leading-tight font-bold tracking-wide text-ink uppercase">{vehicle.nom}</h3>
  ...
  return (
    <section className={clsx('border-l-[3px] py-2 pr-2 pl-2.5', out ? 'border-mission bg-mission-soft/40' : status.state === 'DISPONIBLE' ? 'border-ok' : status.state === 'RESERVE' ? 'border-reserve' : 'border-off')}>
      <div className="mb-1.5 flex items-center gap-2">
        <h4 className="text-[11.5px] font-bold tracking-wide text-slate-600 uppercase" title={hint}>{title}</h4>
...
export function DepartCard({ groupe, vo, p }: { groupe: string; vo: Vehicle; p: Vehicle }) {
  ...
  return (
    <article
      id={`veh-${p.id}`}
      className={clsx(
        'flex flex-col overflow-hidden rounded-lg border bg-white shadow-[0_1px_2px_rgb(15_29_51/0.06)]',
    ...
    >
      ...
      <header className={clsx('flex items-center gap-2 px-3 pt-2 pb-1.5', agg === 'EN_MISSION' && 'bg-mission-soft/50')}>
        <h3 className="font-display text-[19px] leading-tight font-bold tracking-wide text-ink uppercase">{groupe}</h3>
